...
class googleLenetModel:
    def __init__(self):
        ...
        self.is_from_previous = True

        self.img_input = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.height, self.width], name='input')
        ...
        self.full_connect_out1 = self.full_connect_layer(self.w_len, self.fc_len1)
        self.full_connect_out = self.full_connect_layer(self.fc_len1, self.fc_len2) #用于生成哈希码及分类
    ...
    def inception_v2_layer(self):
        ...
        with slim.arg_scope(inception_v2_arg_scope()):
            out, end_points = inception_v2(inputs=self.img_input_expand, num_classes=out_dimension, dropout_keep_prob=self.keep_prob,is_training=self.is_training)
        return out
    ...
    def full_connect_layer(self,in_len, out_len):
        W_fc = tf.Variable(initial_value=tf.random.truncated_normal([in_len, out_len], mean=0, stddev=0.01),name="full_connect_w");
        b_fc = tf.Variable(initial_value=tf.random.truncated_normal([out_len], mean=0, stddev=0.01),name="full_connect_b");
    ...
    def softmax_layer(self):
        W_s = tf.Variable(initial_value=tf.random.truncated_normal([self.fc_len2, self.label_num], mean=0, stddev=0.01));
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for i in range(1, max_epoch):
                ...
                for step,batch in enumerate(train_batch):
                    ...
                    sess.run(self.train_op, feed_dict={self.img_input:img_batch, self.label_id: label_batch, self.keep_prob:0.7, self.is_training:True})
                    ...
                    if step%100 ==0:
                        ...
                        if valid_accuracy > max_accuracy:
                            ...
                            if valid_accuracy > 0.2 and valid_accuracy - self.max_acc_save >= 0.02:
                                ...
                                best_models = "models/best_models_{}_{}_{:.4f}_{}_{}.ckpt".format(i,step, max_accuracy,self.w_len,self.fc_len2)
